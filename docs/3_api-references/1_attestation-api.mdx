...
Every `dimo.document.*` type carries a defined set of fields under `data.fields`. The Extract API returns this shape, and you post the same shape when you store a `dimo.document.*` event. Fields are best-effort: anything the source document does not contain comes back `null`, so a real response often has fewer keys than shown here. Each example below lists the representative fields for one type.
