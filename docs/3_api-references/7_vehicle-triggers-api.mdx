...
Think of it like a "push notification" for servers — instead of you constantly checking the API (polling) for updates,
...
<TwoColumnLayout
  ...
  leftColumn={
    ...
    share, please reach out to <a href="mailto:developersupport@dimo.org">developer-support@dimo.org</a>.
    </div>
  }
  rightColumn={
    <div>
      <BaseUrlBlock url="https://vehicle-triggers-api.dimo.zone"/>
      <ApiEndpointsBlock
        endpoints={[
          {
            method: 'GET',
        ...
        parameters={[
          {
            ...
            type: "string",
            required: true,
            description: "The subsystem producing the metric. Must be one of: signals.vss (VSS vehicle signals), events.behavior (driving behavior events), or events.safety (safety events)"
          },
          {
            name: "metricName",
            type: "string",
            required: true,
            description: "The fully qualified signal or event name to monitor. For signals.vss: speed, powertrainTransmissionTravelledDistance, powertrainTractionBatteryChargingIsCharging, etc. For events.behavior: HarshBraking, HarshAcceleration. For events.safety: collision events."
          },
          {
            name: "condition",
            type: "string",
            ...
            expandable: {
              ...
              content: (
                <div>
                  ...
                  <Link to="https://github.com/DIMO-Network/vehicle-triggers-api?tab=readme-ov-file#cel-conditions">
                    this README
                  </Link>
                </div>
              ),
