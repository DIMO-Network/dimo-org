...
# Conversations API <Badge variant="pre-alpha">Pre-Alpha</Badge>

Create and message AI vehicle agents.

:::danger IMPORTANT
Be aware, the Conversations API is in active development and has not yet been released for development. We recommend developers use this page as a general guide to understanding the general concepts of the API, which will be publicly available in beta soon. Please build at your own risk.
:::
...
<TwoColumnLayout
  leftColumn={
    <div>
      ...
      The DIMO Conversations API enables developers to create intelligent AI agents that can interact with vehicle data through natural language. These agents can query vehicle identity, real-time telemetry, and perform web searches to answer questions about vehicles and nearby services.

      <h3>Key Features</h3>
      <ul>
        <li>Create AI agents with vehicle-specific access</li>
        <li>Query vehicle identity data (make, model, year, owner)</li>
        <li>Access real-time telemetry (speed, fuel, location, battery)</li>
        <li>Perform location-based web searches</li>
        <li>Stream responses in real-time using Server-Sent Events</li>
        <li>Maintain conversation history and context</li>
      </ul>

      <h3>Agent Modes</h3>
      <p><strong>DIMO User Mode:</strong> Agents are created for specific wallet addresses with controlled access to designated vehicles.</p>
      <p><strong>Admin Mode:</strong> Agents have unrestricted access to all vehicles under your developer license, ideal for internal tools and dashboards.</p>
    </div>
  }
  rightColumn={
    <div>
      <BaseUrlBlock url="https://conversations-api.dev.dimo.zone"/>
      <ApiEndpointsBlock
        endpoints={[
          {
            method: 'POST',
...
<TwoColumnLayout
  ...
  leftColumn={
    <div>
      ...
      Creates a new conversational AI agent for a user with optional vehicle access control. Agents are idempotent per user - calling this endpoint multiple times with the same user will return the existing agent.
        ...
        parameters={[
          {
            ...
            type: "string",
            required: true,
            description: "Ethereum wallet address (0x...) for DIMO users, or any identifier (email, name) for admin mode."
          },
          {
            name: "admin",
            type: "boolean",
            required: false,
            description: "Set to true for unrestricted vehicle access. Default: false."
          },
          {
            name: "vehicleIds",
            type: "array",
            required: false,
            description: "Array of vehicle token IDs the agent can access. Use null for unrestricted access (requires ownership validation), or [] for no vehicle access."
          },
          {
            name: "enableWebsearch",
            type: "boolean",
            required: false,
            description: "Enable web search capabilities for finding nearby services. Default: false."
          }
        ]}
      />
    </div>
    ...
    <div>
      <ApiCodeBlock
        ...
        codeExamples={[{
            label: "TypeScript",
            language: "typescript",
            code: `const agent = await dimo.conversations.createAgent({
  ...
  enableWebsearch: true
          ...
          },
          {
            label: "Python",
            language: "python",
            code: `agent = dimo.conversations.create_agent(
  user="0x1234567890abcdef1234567890abcdef12345678",
  ...
  enable_websearch=True
          ...
          },
          {
            label: ".NET",
            language: "csharp",
            code: `var agent = await dimoClient.ConversationsService.CreateAgentAsync(
  User = "0x1234567890abcdef1234567890abcdef12345678",
  VehicleIds = new[] { 872, 1234 },
  EnableWebsearch = true
);`
          },
          {
            label: "cURL",
            language: "shell",
            code: `curl -X POST "https://conversations-api.dev.dimo.zone/agents" \\
...
<TwoColumnLayout
    ...
    <div>
      <ApiCodeBlock
        ...
        codeExamples={[{
            label: "TypeScript",
            language: "typescript",
            code: `const response = await dimo.conversations.sendMessage({
  agentId: "agent-abc123def456",
  message: "What's the make and model of my vehicle?"
          ...
          },
          {
            label: "Python",
            language: "python",
            code: `response = dimo.conversations.send_message(
  ...
  message="What's the make and model of my vehicle?"
          ...
          },
          {
            label: ".NET",
            language: "csharp",
            code: `var response = await dimoClient.ConversationsService.SendMessageAsync(
  AgentId = "agent-abc123def456",
  Message = "What's the make and model of my vehicle?"
);`
          },
          {
            label: "cURL",
            language: "shell",
            code: `curl -X POST "https://conversations-api.dev.dimo.zone/agents/agent-abc123def456/message" \\
  -H "Authorization: Bearer <DEV_JWT>" \\
  -H "Content-Type: application/json" \\
  -d '{
    "message": "What's the make and model of my vehicle?"
    ...
    <div>
      <ApiCodeBlock
        ...
        codeExamples={[{
            label: "TypeScript",
            language: "typescript",
            code: `const stream = dimo.conversations.streamMessage({
  agentId: "agent-abc123def456",
...
stream.on('error', (error) => {
  console.error('Stream error:', error);
          ...
          },
          {
            label: "Python",
            language: "python",
            code: `stream = dimo.conversations.stream_message(
  agent_id="agent-abc123def456",
  message="What's my current speed?"
)
  ...
  if chunk.get('content'):
    ...
    print('\\nDone!')`
          },
          {
            label: ".NET",
            language: "csharp",
            code: `await foreach (var chunk in dimoClient.ConversationsService.StreamMessageAsync(
  AgentId = "agent-abc123def456",
  Message = "What's my current speed?"
))
{
  if (chunk.Content != null)
    ...
    Console.WriteLine("\\nDone!");
          ...
          },
          {
            label: "cURL",
            language: "shell",
            code: `curl -X POST "https://conversations-api.dev.dimo.zone/agents/agent-abc123def456/stream" \\
  -H "Authorization: Bearer <DEV_JWT>" \\
  -H "Content-Type: application/json" \\
    ...
    <div>
      <ApiCodeBlock
        ...
        codeExamples={[{
            label: "TypeScript",
            language: "typescript",
            code: `const history = await dimo.conversations.getHistory({
  agentId: "agent-abc123def456",
  limit: 50
})`
          },
          {
            label: "Python",
            language: "python",
            code: `history = dimo.conversations.get_history(
  agent_id="agent-abc123def456",
  limit=50
)`
          },
          {
            label: ".NET",
            language: "csharp",
            code: `var history = await dimoClient.ConversationsService.GetHistoryAsync(
  AgentId = "agent-abc123def456",
  Limit = 50
);`
          },
          {
            label: "cURL",
            language: "shell",
            code: `curl -X GET "https://conversations-api.dev.dimo.zone/agents/agent-abc123def456/history?limit=50" \\
  -H "Authorization: Bearer <DEV_JWT>"`
          }
        ]}
        defaultLanguage="typescript"
    ...
    <div>
      <ApiCodeBlock
        ...
        codeExamples={[{
            label: "TypeScript",
            language: "typescript",
            code: `await dimo.conversations.deleteAgent({
  agentId: "agent-abc123def456"
})`
          },
          {
            label: "Python",
            language: "python",
            code: `dimo.conversations.delete_agent(
  agent_id="agent-abc123def456"
)`
          },
          {
            label: ".NET",
            language: "csharp",
            code: `await dimoClient.ConversationsService.DeleteAgentAsync(
  AgentId = "agent-abc123def456"
);`
          },
          {
            label: "cURL",
            language: "shell",
            code: `curl -X DELETE "https://conversations-api.dev.dimo.zone/agents/agent-abc123def456" \\
  -H "Authorization: Bearer <DEV_JWT>"`
          }
        ]}
        defaultLanguage="typescript"
