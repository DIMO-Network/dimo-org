...
Use the playground to try the four root queries: <code>latestIndex</code>, <code>indexes</code>, <code>latestCloudEvent</code>, and <code>cloudEvents</code>. Replace the <code>did</code> placeholder with a valid DID you have access to.
...
**DID**: The <code>did</code> argument accepts any DID in the DIMO system (e.g. <code>did:eth:1:0xContractAddress:tokenId</code>). You can resolve it from the Identity API or your token exchange flow.
