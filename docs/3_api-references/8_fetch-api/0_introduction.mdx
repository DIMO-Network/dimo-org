---
sidebar_position: 1
description: Retrieve raw vehicle data objects and index keys from DIMO cloud storage with the Fetch API.
---
...
<TwoColumnLayout
  leftColumn={
    ...
    The <code>did</code> parameter accepts any DID in the DIMO system (e.g. a vehicle DID like <code>did:erc721:1:0xContractAddress:tokenId</code>).
    </div>
  }
  rightColumn={
    <div>
...
<TwoColumnLayout
  ...
  leftColumn={
    <div>
      ...
      <h4>CloudEventTypeSummary</h4>
      Returned by <code>availableCloudEventTypes</code>. Use it to discover which CloudEvent types exist for a subject before you commit to a full query.
